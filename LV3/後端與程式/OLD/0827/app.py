...
def load_month_data_db(year, month):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            'SELECT chip_id, user, date, model, grade, defect, jnd,'
            ' description, judgment, tool_id, pol,'
            ' x_start, x_end, y_start, y_end, image_path FROM records'
        )
...
def parse_source(source_str):
    if source_str and source_str.strip().lower().endswith('.png'):
        return {'file_path': source_str.strip(), 'sheet_name': '', 'row_num': 0}
    try:
                ...
                for year_item, month in query_periods:
                    ...
                    for record in records:
                        ...
                        if not apply_adv_filter(record, adv_filter):
                            continue
                        source_info = parse_source(record.get('source', ''))
