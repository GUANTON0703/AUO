...
def api_log_clear():
    ...
    try:
        ...
        db.commit()
        db.close()
        wlog("Clear access_log: " + str(db_deleted) + " rows deleted")
    except Exception as e:
        wlog("api_log_clear db error: " + str(e))

    # 2) Clear .log files in LOG_DIR (keep launcher.log intact, truncate others)
    cleared_files = []
    skipped_files = []
    try:
        if os.path.isdir(LOG_DIR):
            for fname in os.listdir(LOG_DIR):
                if not fname.endswith(".log"):
                    continue
                if fname == "launcher.log":
                    continue
                fpath = os.path.join(LOG_DIR, fname)
                try:
                    open(fpath, "w", encoding="utf-8").close()
                    cleared_files.append(fname)
                    wlog("Cleared log file: " + fname)
                except Exception as fe:
                    skipped_files.append(fname)
                    wlog("Failed to clear " + fname + ": " + str(fe))
    except Exception as e:
        wlog("api_log_clear files error: " + str(e))
