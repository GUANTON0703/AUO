...
LOCK_FILE     = r"\\tw100049089\00_MyAgent\Python\LOG\HTA.lock"
LOG_DIR       = r"\\tw100049089\00_MyAgent\Python\LOG"
DB_PATH       = os.path.join(LOG_DIR, "hta_access.db")
NAME_MAP_PATH = r"\\tw100049089\00_MyAgent\後端總控台\name_map.json"
MGMT_PORT     = 5100
...
def _get_thread_session(loop):
    if not hasattr(_thread_local, "session") or _thread_local.session.closed:
        connector = aiohttp.TCPConnector(
            limit=PROXY_POOL_SIZE, limit_per_host=PROXY_POOL_CONN,
            ttl_dns_cache=300, loop=loop
        )
...
def _start_proxy_thread(svc):
    ...
    def _run():
        ...
        serve(proxy_app, host="0.0.0.0", port=real_port, threads=16, channel_timeout=30, cleanup_interval=10)
