...
【v17.1 修正】
  _serve_file 與 _respond 加入 gzip 壓縮，解決 WinError 10053 大檔傳輸斷線問題。
"""
...
def _serve_file(handler, filepath, content_type):
    ...
    try:
        ...
        msg = ("找不到檔案：" + filepath).encode("utf-8")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/fetch":
            ...
            try:
                ...
                print(f"  隱藏欄位取得完成（{len(hidden)} 個），開始並行抓取...")
    ...
    def _respond(self, body):
        """JSON 回應 — v17.1 加入 gzip 壓縮"""
        ae = self.headers.get("Accept-Encoding", "")
        if "gzip" in ae:
            out = gzip.compress(body, compresslevel=6)
            enc = "gzip"
        else:
            out = body
            enc = None
...
def run():
    ...
    print("=" * 60)
    print(f"  C1 橋接程式 v17.1 已啟動  (port {port})")
    print()
