...
(function(){
    ...
    if(typeof origBqSearch === 'function'){
        ...
        var src = origBqSearch.toString().replace(/localhost:5000/g, '');
...
function applyFixes(){
    ...
    setTimeout(function(){
        var el = document.getElementById('bqOnlineStatus');
        if(!el) return;
        fetch('/ping', { signal: AbortSignal.timeout(2500), cache:'no-store' })
            .then(function(r){ return r.json(); })
            .then(function(){ el.textContent='🟢 後端在線'; el.className='bq-online'; })
            .catch(function(){ el.textContent='⚫ 後端離線'; el.className='bq-offline'; });
    }, 500);
...
/* [patch] 隱藏舊版 regionGrid（由 pi_patch2.js Canvas 取代）*/
(function(){
    function hideOldGrid(){
        var rg = document.getElementById('regionGrid');
        if(rg){ rg.style.display = 'none'; }

        /* Hook updateRegionGrid：執行後順帶更新計數 Canvas */
        var origURG = window.updateRegionGrid;
        if(typeof origURG === 'function' && !origURG.__p2hooked){
            window.updateRegionGrid = function(){
                origURG.apply(this, arguments);
                if(typeof window.__p2DrawCount === 'function') window.__p2DrawCount();
            };
            window.updateRegionGrid.__p2hooked = true;
        }
    }
    if(document.readyState === 'complete') setTimeout(hideOldGrid, 600);
    else window.addEventListener('load', function(){ setTimeout(hideOldGrid, 600); });
})();


/* [patch] 隱藏舊版 checkbox 篩選（由 pi_patch2.js Canvas 取代）*/
(function(){
    function hideOldFilter(){
        /* buildFilterUI 產生的容器 id 可能是 regionFilter 或 filterGroup */
        ['regionFilter','filterGroup','regionFilterWrap'].forEach(function(id){
            var el = document.getElementById(id);
            if(el){ el.style.display = 'none'; }
        });
        /* 也隱藏含 checkbox[name="region"] 的整個 control-group */
        document.querySelectorAll('input[type=checkbox]').forEach(function(cb){
            if(cb.name === 'region' || cb.dataset.region){
                var grp = cb.closest('.control-group');
                if(grp && !grp.id.startsWith('p2_')){
                    grp.style.display = 'none';
                }
            }
        });
    }
    if(document.readyState === 'complete') setTimeout(hideOldFilter, 700);
    else window.addEventListener('load', function(){ setTimeout(hideOldFilter, 700); });
})();

/* ─────────────────────────────── */
/* bqSearch v_batch：改用 /query_batch，單次請求查所有 ID */
/* 前端只發 1 個 fetch，避免平行請求各自超時的問題       */
(function(){
'use strict';

window.bqSearch = function(){
    var PROXY = '';   // 同機器，不需要 host
    ...
    if(btn){ btn.disabled = true; btn.textContent = '⏳ 查詢中...'; }
    var bqS = document.getElementById('bqStatus');
    if(bqS){ bqS.textContent = '🔄 後端批次查詢中（共 ' + sheetIDs.length + ' 個ID），請稍候...'; bqS.className = 'loading'; bqS.style.display = 'block'; }

    // ★ 所有 ID 合成一個請求，timeout 120 秒
    var url = PROXY + '/query_batch?sheet_ids=' + encodeURIComponent(sheetIDs.join(',')) +
              '&start_date=' + encodeURIComponent(start) +
              '&end_date='   + encodeURIComponent(end) +
              '&image_type=' + encodeURIComponent(imgType) +
              '&id_type='    + encodeURIComponent(idType);
    ...
    fetch(url, { signal: AbortSignal.timeout(120000) })
        ...
        .then(function(data){
            ...
            if(data.error){
                if(bqS){ bqS.textContent = '❌ ' + data.error; bqS.className = 'err'; bqS.style.display = 'block'; }
                return;
            }

            var pts = data.points || [];
            if(!pts.length){
                if(bqS){ bqS.textContent = '❌ 查無資料' + (data.warning ? '\n' + data.warning : ''); bqS.className = 'err'; bqS.style.display = 'block'; }
                return;
            }

            // 轉格式 → 寫入原始 textarea → 觸發匯入
            var lines = pts.map(function(p){
                return p.sheet_id + '|' + p.url + '\t' + p.line_id + '\t' + p.x + '\t' + p.y;
            });
            var origTA = Array.from(document.querySelectorAll('textarea')).find(function(t){
                return t.id !== 'spPasteArea' && t.id !== 'bqSheetID';
            });
            if(origTA) origTA.value = lines.join('\n');
            ...
            var msg = '✅ 查詢完成！共 ' + pts.length + ' 筆已自動標點';
            if(data.errors && data.errors.length)
                msg += '\n⚠️ 部分失敗：' + data.errors.join(' / ');
            if(bqS){ bqS.textContent = msg; bqS.className = 'ok'; bqS.style.display = 'block'; }
        })
        .catch(function(e){
            if(btn){ btn.disabled = false; btn.textContent = '🔍 搜尋並自動標點'; }
            var msg = e.name === 'TimeoutError'
                ? '❌ 請求超時（>120秒），請稍後重試或確認後端在線'
                : '❌ 無法連線：' + e.message;
            if(bqS){ bqS.textContent = msg; bqS.className = 'err'; bqS.style.display = 'block'; }
        });
};

console.log('[bqSearch v_batch] 已覆蓋，使用 /query_batch 單次請求 ✓');
})();

/* ─────────────────────────────── */
/* bqSearch v_batch_fix：修正標點問題                    */
/* 直接用 #dataInput + 直接呼叫 loadData()，不動態搜尋    */
(function(){
'use strict';
...
window.bqSearch = function(){
    ...
    var bqS = document.getElementById('bqStatus');
    if(bqS){
        bqS.textContent = '🔄 批次查詢中（共 ' + sheetIDs.length + ' 個ID），請稍候...';
        bqS.className = 'loading';
        bqS.style.display = 'block';
    }
    ...
    fetch(url, { signal: AbortSignal.timeout(120000) })
        ...
        .then(function(data){
            ...
            if(!pts.length){
                ...
                if(bqS){ bqS.textContent = '❌ 查無資料' + (warn ? '\n' + warn : ''); bqS.className = 'err'; bqS.style.display = 'block'; }
                return;
            }

            // ★ 轉成 loadData() 讀得懂的格式
            //   格式：SHEET_ID|URL \t LINE_ID \t X \t Y
            //   X、Y 維持原始值（loadData 會除以 1000）
            var lines = pts.map(function(p){
                return p.sheet_id + '|' + (p.url || '') + '\t' + (p.line_id || '') + '\t' + p.x + '\t' + p.y;
            });

            // ★ 直接寫入 #dataInput（不動態搜尋 textarea）
            var ta = document.getElementById('dataInput');
            if(!ta){
                if(bqS){ bqS.textContent = '❌ 找不到 #dataInput，請確認頁面結構'; bqS.className = 'err'; bqS.style.display = 'block'; }
                return;
            }
            ta.value = lines.join('\n');

            // ★ 直接呼叫 loadData()（不動態搜尋按鈕）
            if(typeof window.loadData === 'function'){
                window.loadData();
            } else {
                if(bqS){ bqS.textContent = '❌ loadData() 不存在，請確認 pi_main.js 已載入'; bqS.className = 'err'; bqS.style.display = 'block'; }
                return;
            }

            // ★ 標點成功後，把所有點加入 allMarkedIndices（顯示紅點）
            //   loadData() 只載入 allPoints，不自動標紅
            //   但這裡的 PI 查詢結果本身就是不良點，應全部標紅
            setTimeout(function(){
                if(typeof window.allPoints !== 'undefined' &&
                   typeof window.allMarkedIndices !== 'undefined'){
                    window.allMarkedIndices = window.allPoints.map(function(_, i){ return i; });
                    window.loadedRedPoints  = window.allPoints.map(function(p, i){
                        return Object.assign({}, p, {_allIdx: i});
                    });
                    if(typeof window.markedPoints !== 'undefined'){
                        window.markedPoints = new Set(window.allMarkedIndices);
                    }
                    if(typeof window.updateDisplay          === 'function') window.updateDisplay();
            ...
            var msg = '✅ 查詢完成！共 ' + pts.length + ' 筆已標點';
            if(data.errors && data.errors.length)
                msg += '\n⚠️ 部分失敗：' + data.errors.join(' / ');
            if(bqS){ bqS.textContent = msg; bqS.className = 'ok'; bqS.style.display = 'block'; }
        })
...
console.log('[bqSearch v_batch_fix] 已覆蓋 ✓');
})();

/* ─────────────────────────────── */
/* drawBackground v_fix_cache：
   修正 _bgImg 宣告在函數內導致快取失效 + updateDisplay 無窮迴圈 */
(function(){
'use strict';

// ★ 快取放在 IIFE 閉包（函數外），才能真正跨呼叫保留
let _bgCache = {};          // key: imgSrc → Image 物件（已載完）
let _bgLoading = {};        // key: imgSrc → true，避免重複 fetch
let _bgFailed  = {};        // key: imgSrc → true，404 不重試

function drawBackgroundFixed_v2(){
    const isDark = document.body.classList.contains('dark-mode');
    const canvas = document.getElementById('canvas');
    if(!canvas) return;
    const ctx = canvas.getContext('2d');
    ...
    // ── 畫 chip 格線 ──────────────────────────────
    function doDrawLines(){
        const SC = canvas.width / (typeof REAL_WIDTH  !== 'undefined' ? REAL_WIDTH  :
                   typeof REAL_W      !== 'undefined' ? REAL_W      : 2500);
        const REAL_H_val = typeof REAL_HEIGHT !== 'undefined' ? REAL_HEIGHT :
                           typeof REAL_H      !== 'undefined' ? REAL_H      : 2200;
        ...
        (prod.chips || []).forEach(chip => {
            const x1 = chip.x * SC,  x2 = (chip.x + chip.w) * SC;
            const y1 = canvas.height - chip.y * SC;
            const y2 = canvas.height - (chip.y + chip.h) * SC;
            const RW = typeof REAL_WIDTH !== 'undefined' ? REAL_WIDTH : REAL_W || 2500;

            if(chip.x > 0.5){
                ctx.beginPath(); ctx.moveTo(x1,y2); ctx.lineTo(x1,y1); ctx.stroke();
            }
            if(chip.x + chip.w < RW - 0.5){
                ctx.beginPath(); ctx.moveTo(x2,y2); ctx.lineTo(x2,y1); ctx.stroke();
            }
            if(chip.y > 0.5){
                ctx.beginPath(); ctx.moveTo(x1,y1); ctx.lineTo(x2,y1); ctx.stroke();
            }
            if(chip.y + chip.h < REAL_H_val - 0.5){
                ctx.beginPath(); ctx.moveTo(x1,y2); ctx.lineTo(x2,y2); ctx.stroke();
            }
        });
        ...
        (prod.chips || []).forEach(c => {
            const SC2 = canvas.width / (typeof REAL_WIDTH !== 'undefined' ? REAL_WIDTH :
                        typeof REAL_W !== 'undefined' ? REAL_W : 2500);
            const REAL_H2 = typeof REAL_HEIGHT !== 'undefined' ? REAL_HEIGHT :
                            typeof REAL_H !== 'undefined' ? REAL_H : 2200;
            const cxPx = (c.x + c.w * 0.5) * SC2;
            const cyPx = canvas.height - (c.y + c.h * 0.5) * SC2;
            const fs = Math.max(6, Math.min(c.w*SC2*0.25, c.h*SC2*0.25, 14));
            ctx.font = `bold ${fs}px sans-serif`;
            ctx.fillStyle = isDark ? 'rgba(170,205,255,0.6)' : 'rgba(0,60,140,0.4)';
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
    ...
    // 正在載入中 → 不重複發請求
    if(_bgLoading[imgSrc]) return;
    _bgLoading[imgSrc] = true;

    const img = new Image();
    img.onload = () => {
        _bgCache[imgSrc]   = img;
        _bgLoading[imgSrc] = false;
        // ★ 用 requestAnimationFrame 重繪，不呼叫 updateDisplay()（避免迴圈）
        requestAnimationFrame(drawBackgroundFixed_v2);
    };
    img.onerror = () => {
        _bgFailed[imgSrc]  = true;
        _bgLoading[imgSrc] = false;
        console.warn('[drawBackground] 底圖載入失敗（404？）:', imgSrc);
        // 已畫好虛線，不需重繪
    };
    img.src = imgSrc;
}
...
function tryInstall(){
    if(typeof PRODUCT_CONFIGS !== 'undefined'){
        window.drawBackground = drawBackgroundFixed_v2;
        drawBackgroundFixed_v2();
        console.log('[drawBackground v_fix_cache] 安裝完成 ✓');
    } else {
        setTimeout(tryInstall, 200);
    }
}

if(document.readyState === 'complete') setTimeout(tryInstall, 350);
else window.addEventListener('load', () => setTimeout(tryInstall, 350));
...
/* ═══════════════════════════════════════════════════════════ */
/* bqSearch v_cf_auto：TFT / CF 自動偵測 + 上下翻轉            */
/* ═══════════════════════════════════════════════════════════ */
(function(){
'use strict';
...
/* ── 側別 badge（畫面右上角常駐）─────────────────────────── */
var _sideBadge = null;
function ensureSideBadge(){
    if(_sideBadge) return _sideBadge;
    _sideBadge = document.createElement('div');
    _sideBadge.id = 'sideBadge';
    _sideBadge.style.cssText = [
    ...
    document.body.appendChild(_sideBadge);
    return _sideBadge;
}

function setSideBadge(side){
    var b = ensureSideBadge();
    if(side === 'TFT'){
        b.textContent = '🔵 TFT 側';
        b.style.background = 'rgba(0,80,200,0.85)';
        b.style.color = '#fff';
        b.style.display = 'block';
    } else if(side === 'CF'){
        b.textContent = '🔴 CF 側（已上下翻轉）';
        b.style.background = 'rgba(180,30,30,0.85)';
        b.style.color = '#fff';
        b.style.display = 'block';
    } else {
        b.textContent = '⚪ 側別未知';
        b.style.background = 'rgba(100,100,100,0.75)';
        b.style.color = '#fff';
        b.style.display = 'block';
    }
}
...
window.bqSearch = function(){
    ...
    if(btn){ btn.disabled = true; btn.textContent = '⏳ 查詢中...'; }
    if(bqS){
        bqS.textContent = '🔄 查詢中（' + sheetIDs.length + ' 個ID + 自動偵測TFT/CF）...';
        bqS.className = 'loading'; bqS.style.display = 'block';
    }

    /* ── 1. 平行發出：/identify（只查第一片）+ /query_batch（全部）─── */
    var firstId  = sheetIDs[0];
    var allIds   = sheetIDs.join(',');

    var identUrl = '/identify?ids=' + encodeURIComponent(firstId);
    var batchUrl = '/query_batch'
        + '?sheet_ids='  + encodeURIComponent(allIds)
        + '&start_date=' + encodeURIComponent(start)
        + '&end_date='   + encodeURIComponent(end)
        + '&image_type=' + encodeURIComponent(imgType)
        + '&id_type='    + encodeURIComponent(idType);

    var identPromise = fetch(identUrl, {signal: AbortSignal.timeout(60000)})
        .then(function(r){ return r.json(); })
        .catch(function(){ return null; });   // identify 失敗不阻塞主流程

    var batchPromise = fetch(batchUrl, {signal: AbortSignal.timeout(120000)})
        .then(function(r){ return r.json(); });

    Promise.all([identPromise, batchPromise])
    .then(function(results){
        var identData = results[0];
        ...
        /* ── 2. 判斷側別 ──────────────────────────────────────── */
        var side = 'unknown';
        if(identData && identData.results && identData.results.length){
            side = identData.results[0].side || 'unknown';
        }
        window.inspSide = side;
        setSideBadge(side);

        /* ── 3. 處理查詢結果 ──────────────────────────────────── */
        if(batchData.error){
            if(bqS){ bqS.textContent = '❌ ' + batchData.error; bqS.className='err'; bqS.style.display='block'; }
            return;
        }

        var pts = batchData.points || [];
        if(!pts.length){
            if(bqS){
                bqS.textContent = '❌ 查無資料' + (batchData.warning ? '\n'+batchData.warning : '');
                bqS.className='err'; bqS.style.display='block';
            }
            return;
        }

        /* ── 4. 寫入 #dataInput，呼叫 loadData() ─────────────── */
        var lines = pts.map(function(p){
            return p.sheet_id + '|' + (p.url||'') + '\t' + (p.line_id||'') + '\t' + p.x + '\t' + p.y;
        });
        var ta = document.getElementById('dataInput');
        if(!ta){
            if(bqS){ bqS.textContent = '❌ 找不到 #dataInput'; bqS.className='err'; bqS.style.display='block'; }
            return;
        }
        ta.value = lines.join('\n');
        ...
        if(typeof window.loadData === 'function'){
            ...
            if(bqS){ bqS.textContent = '❌ loadData() 不存在'; bqS.className='err'; bqS.style.display='block'; }
            return;
        }

        /* ── 5. 延後處理：CF 翻轉 + 標紅 + 更新顯示 ─────────── */
        setTimeout(function(){
            /* 5a. CF 側 → Y 座標上下翻轉 */
            if(side === 'CF'){
                // REAL_H：全局優先，抓不到就從 canvas 反推，最後 fallback 2200
                var canvas = document.getElementById('canvas');
                var realH  = (typeof window.REAL_HEIGHT !== 'undefined') ? window.REAL_HEIGHT
                           : (typeof window.REAL_H      !== 'undefined') ? window.REAL_H
                           : canvas
                             ? canvas.height / (canvas.width / ((typeof window.REAL_WIDTH !== 'undefined') ? window.REAL_WIDTH : 2500))
                             : 2200;

                if(window.allPoints && window.allPoints.length){
                    window.allPoints = window.allPoints.map(function(p){
                        return Object.assign({}, p, { y: realH - p.y });
                    });
                }
            }

            /* 5b. 全部標紅（PI 查回來的都是不良點） */
            if(typeof window.allPoints !== 'undefined'){
                window.allMarkedIndices = window.allPoints.map(function(_, i){ return i; });
                window.loadedRedPoints  = window.allPoints.map(function(p, i){
                    return Object.assign({}, p, { _allIdx: i });
            ...
            /* 5d. 狀態訊息 */
            var sideLabel = side === 'TFT' ? '🔵 TFT 側'
                          : side === 'CF'  ? '🔴 CF 側（已上下翻轉）'
                          :                  '⚪ 側別偵測失敗（按 TFT 處理）';
            var msg = '✅ 查詢完成！共 ' + pts.length + ' 筆　' + sideLabel;
            if(batchData.errors && batchData.errors.length)
                msg += '\n⚠️ 部分失敗：' + batchData.errors.join(' / ');
            if(bqS){ bqS.textContent = msg; bqS.className='ok'; bqS.style.display='block'; }
...
window.clearAll = function(){
    if(_origClearAll) _origClearAll();
    window.inspSide = null;
    var b = document.getElementById('sideBadge');
    if(b) b.style.display = 'none';
};

console.log('[bqSearch v_cf_auto] TFT/CF 自動偵測已啟用 ✓');
})();

/* ═══════════════════════════════════════════════════════════ */
/* bqSearch v_cf_inplace：in-place 修改陣列，修正 scope 問題    */
/* ═══════════════════════════════════════════════════════════ */
(function(){
'use strict';

/* ── 側別 badge（沿用之前的 ensureSideBadge，若不存在才建）── */
function setSideBadge(side){
    var b = document.getElementById('sideBadge');
    if(!b){
    ...
    if(side === 'TFT'){
        b.textContent = '🔵 TFT 側'; b.style.cssText += ';background:rgba(0,80,200,0.85);color:#fff;display:block;';
    } else if(side === 'CF'){
        b.textContent = '🔴 CF 側（已上下翻轉）'; b.style.cssText += ';background:rgba(180,30,30,0.85);color:#fff;display:block;';
    } else {
        b.textContent = '⚪ 側別未知'; b.style.cssText += ';background:rgba(100,100,100,0.75);color:#fff;display:block;';
    }
}
...
window.bqSearch = function(){
    ...
    /* 平行發出 identify（只查第一片）和 query_batch（全部） */
    var firstId  = sheetIDs[0];
    var allIds   = sheetIDs.join(',');

    var identPromise = fetch('/identify?ids='+encodeURIComponent(firstId),
        {signal:AbortSignal.timeout(60000)})
        .then(function(r){ return r.json(); }).catch(function(){ return null; });

    var batchPromise = fetch(
        '/query_batch?sheet_ids='+encodeURIComponent(allIds)
        +'&start_date='+encodeURIComponent(start)
        +'&end_date='+encodeURIComponent(end)
        +'&image_type='+encodeURIComponent(imgType)
        +'&id_type='+encodeURIComponent(idType),
    ...
    .then(function(res){
        ...
        var pts = batchData.points || [];
        if(!pts.length){
            if(bqS){ bqS.textContent='❌ 查無資料'+(batchData.warning?'\n'+batchData.warning:'');
                     bqS.className='err'; bqS.style.display='block'; }
            return;
        }

        /* ── 寫入 #dataInput + loadData() */
        var lines = pts.map(function(p){
            return p.sheet_id+'|'+(p.url||'')+'\t'+(p.line_id||'')+'\t'+p.x+'\t'+p.y;
        });
        var ta = document.getElementById('dataInput');
        ...
        /* ── 延後處理（等 loadData 內部同步完成） */
        setTimeout(function(){

            /* ★ Bug1 修正：in-place 翻轉 y，不替換陣列 reference ★
               .map() 會產生新陣列 → pi_main.js local var 看不到
               改用 .forEach 直接改物件的 y 屬性 → 同一個 object ref，
               pi_main.js 的 allPoints、loadedRedPoints 全都看到變化  */

            var realH = (typeof window.REAL_HEIGHT !== 'undefined') ? window.REAL_HEIGHT
                      : (typeof window.REAL_H      !== 'undefined') ? window.REAL_H
                      : (function(){
                            var cv = document.getElementById('canvas');
                            var rw = (typeof window.REAL_WIDTH!=='undefined') ? window.REAL_WIDTH : 2500;
                            return cv ? cv.height / (cv.width / rw) : 2200;
                        })();

            if(side === 'CF' && window.allPoints && window.allPoints.length){
                /* allPoints：y 原地翻轉 */
                window.allPoints.forEach(function(p){ p.y = realH - p.y; });
                /* loadedRedPoints（若存在）：同步翻轉 */
                if(window.loadedRedPoints){
                    window.loadedRedPoints.forEach(function(p){
                        if(typeof p.y === 'number') p.y = realH - p.y;
                    });
                }
            }

            /* ★ 標紅：in-place 清空再 push，不替換 allMarkedIndices reference ★ */
            if(window.allMarkedIndices){
                window.allMarkedIndices.length = 0;   // 清空，保留 reference
                (window.allPoints||[]).forEach(function(_,i){ window.allMarkedIndices.push(i); });
            } else {
                window.allMarkedIndices = (window.allPoints||[]).map(function(_,i){ return i; });
            }
            if(window.markedPoints){
                window.markedPoints.clear();
                (window.allMarkedIndices||[]).forEach(function(i){ window.markedPoints.add(i); });
            } else {
                window.markedPoints = new Set(window.allMarkedIndices||[]);
            }
            /* loadedRedPoints：in-place 重建 */
            if(window.loadedRedPoints){
                window.loadedRedPoints.length = 0;
                (window.allPoints||[]).forEach(function(p,i){
                    window.loadedRedPoints.push(Object.assign({}, p, {_allIdx:i}));
                });
            } else {
                window.loadedRedPoints = (window.allPoints||[]).map(function(p,i){
                    return Object.assign({}, p, {_allIdx:i});
    ...
    .catch(function(e){
        if(btn){ btn.disabled=false; btn.textContent='🔍 搜尋並自動標點'; }
        if(bqS){
            bqS.textContent = (e.name==='TimeoutError' ? '❌ 請求超時（>120秒）' : '❌ 連線失敗：'+e.message);
            bqS.className='err'; bqS.style.display='block';
        }
    });
};
...
console.log('[bqSearch v_cf_inplace] ✓');
})();


/* ═══════════════════════════════════════════════════════════ */
/* drawBackground v_cf_image：CF 模式底圖上下翻轉               */
/* 安裝在 T+500ms（比 v_fix_cache 的 350ms 晚），確保最後生效   */
/* ═══════════════════════════════════════════════════════════ */
(function(){
'use strict';

var _bgCache   = {};   // imgSrc → Image（已載入）
var _bgLoading = {};   // imgSrc → bool
var _bgFailed  = {};   // imgSrc → bool

function drawBG_final(){
    var isDark = document.body.classList.contains('dark-mode');
    var canvas = document.getElementById('canvas');
    if(!canvas) return;
    var ctx = canvas.getContext('2d');
    ...
    if(typeof PRODUCT_CONFIGS === 'undefined' || typeof currentProductKey === 'undefined') return;
    var prod = PRODUCT_CONFIGS[currentProductKey];
    if(!prod) return;

    var isCF   = (window.inspSide === 'CF');
    var imgSrc = prod.image;
    var SC     = canvas.width / ((typeof REAL_WIDTH!=='undefined') ? REAL_WIDTH
                                :(typeof REAL_W    !=='undefined') ? REAL_W : 2500);
    var REAL_H_val = (typeof REAL_HEIGHT!=='undefined') ? REAL_HEIGHT
                   : (typeof REAL_H    !=='undefined') ? REAL_H : 2200;

    /* ── 畫 chip 格線 + 標籤（不隨 CF 翻轉，座標已換到 TFT 空間）── */
    function doDrawLines(){
        ctx.save();
        ctx.strokeStyle = 'rgba(210,40,40,0.55)';
        ctx.lineWidth = 1;
        ...
        (prod.chips || []).forEach(function(chip){
            var x1 = chip.x * SC,   x2 = (chip.x + chip.w) * SC;
            var y1 = canvas.height - chip.y * SC;
            var y2 = canvas.height - (chip.y + chip.h) * SC;
            var RW = (typeof REAL_WIDTH!=='undefined') ? REAL_WIDTH
                   : (typeof REAL_W    !=='undefined') ? REAL_W : 2500;
            if(chip.x > 0.5){
                ctx.beginPath(); ctx.moveTo(x1,y2); ctx.lineTo(x1,y1); ctx.stroke();
            }
        ...
        (prod.chips || []).forEach(function(c){
            var cxPx = (c.x + c.w*0.5) * SC;
            var cyPx = canvas.height - (c.y + c.h*0.5) * SC;
            var fs = Math.max(6, Math.min(c.w*SC*0.25, c.h*SC*0.25, 14));
            ctx.font = 'bold '+fs+'px sans-serif';
            ctx.fillStyle = isDark ? 'rgba(170,205,255,0.6)' : 'rgba(0,60,140,0.4)';
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
    ...
    /* ── 底圖繪製（CF 模式：垂直翻轉）───────────────────────── */
    function drawImgNormal(img){
        ctx.globalAlpha = 0.4;
        ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
        ctx.globalAlpha = 1;
    }
    function drawImgCF(img){
        /* 上下鏡射：translate to bottom, scale y=-1，再畫 */
        ctx.save();
    ...
    function drawImg(img){ isCF ? drawImgCF(img) : drawImgNormal(img); }

    if(!imgSrc){ doDrawLines(); return; }

    /* 已快取 → 直接畫 */
    if(_bgCache[imgSrc]){
        drawImg(_bgCache[imgSrc]);
        doDrawLines();
        return;
    }
    /* 已知 404 → 只畫線 */
    if(_bgFailed[imgSrc]){ doDrawLines(); return; }
    /* 先畫線（底圖非同步載入中） */
    doDrawLines();
    if(_bgLoading[imgSrc]) return;
    _bgLoading[imgSrc] = true;
    ...
    img.onload = function(){
        _bgCache[imgSrc]   = img;
        _bgLoading[imgSrc] = false;
        requestAnimationFrame(drawBG_final);   /* 重繪（不呼叫 updateDisplay 避免迴圈）*/
    };
    img.onerror = function(){
        _bgFailed[imgSrc]  = true;
        _bgLoading[imgSrc] = false;
        console.warn('[drawBG] 底圖載入失敗:', imgSrc);
    };
    img.src = imgSrc;
}

/* 安裝（等 PRODUCT_CONFIGS 和前面所有版本都執行完） */
function install(){
    if(typeof PRODUCT_CONFIGS === 'undefined'){
        setTimeout(install, 200); return;
    }
    window.drawBackground = drawBG_final;
    drawBG_final();
    console.log('[drawBackground v_cf_image] ✓（CF底圖翻轉已啟用）');
}

/* T+500ms 確保最後安裝 */
if(document.readyState === 'complete') setTimeout(install, 500);
else window.addEventListener('load', function(){ setTimeout(install, 500); });
...
/* ═══════════════════════════════════════════════════════════════════
   CF v2 完整修正：整體翻轉策略
   allPoints.y 不翻轉，顯示層統一處理 CF 翻轉
   ═══════════════════════════════════════════════════════════════════ */
(function(){
'use strict';

function isCF(){ return window.inspSide === 'CF'; }

/* ── Badge ── */
function setSideBadge(side){
    var b = document.getElementById('sideBadge');
    if(!b){
        b = document.createElement('div'); b.id='sideBadge';
        b.style.cssText='position:fixed;top:8px;right:12px;z-index:9999;'
            +'padding:4px 10px;border-radius:12px;font-size:13px;font-weight:bold;'
            +'letter-spacing:1px;pointer-events:none;transition:background .3s;';
        document.body.appendChild(b);
    }
    if(side==='TFT'){       b.textContent='🔵 TFT 側'; b.style.background='rgba(0,80,200,0.85)'; }
    else if(side==='CF'){   b.textContent='🔴 CF 側';  b.style.background='rgba(180,30,30,0.85)';}
    else{                   b.textContent='⚪ 側別未知'; b.style.background='rgba(100,100,100,0.75)';}
    b.style.color='#fff'; b.style.display='block';
}

/* ══════════════════════════════════════════════════
   ① bqSearch v_cf_final（不翻轉 allPoints.y）
   ══════════════════════════════════════════════════ */
window.bqSearch = function(){
    var sheetRaw=(document.getElementById('bqSheetID')||{}).value||'';
    var start=(document.getElementById('bqStartDate')||{}).value||'';
    ...
    var idType=(document.getElementById('bqIDType')||{}).value||'SHEET';
    var ids=sheetRaw.split(/[\n,;，；\t]+/).map(function(s){return s.trim();}).filter(Boolean);
    if(!ids.length){ var s=document.getElementById('bqStatus'); if(s){s.textContent='⚠️ 請輸入 SHEET_ID';s.className='warn';s.style.display='block';} return; }

    var btn=document.getElementById('bqSearchBtn'), bqS=document.getElementById('bqStatus');
    if(btn){btn.disabled=true;btn.textContent='⏳ 查詢中...';}
    if(bqS){bqS.textContent='🔄 查詢中（'+ids.length+' 個ID）...';bqS.className='loading';bqS.style.display='block';}

    var identP=fetch('/identify?ids='+encodeURIComponent(ids[0]),{signal:AbortSignal.timeout(60000)})
        .then(function(r){return r.json();}).catch(function(){return null;});
    var batchP=fetch('/query_batch?sheet_ids='+encodeURIComponent(ids.join(','))
        +'&start_date='+encodeURIComponent(start)+'&end_date='+encodeURIComponent(end)
        +'&image_type='+encodeURIComponent(imgType)+'&id_type='+encodeURIComponent(idType),
        {signal:AbortSignal.timeout(120000)}).then(function(r){return r.json();});

    Promise.all([identP,batchP]).then(function(res){
        if(btn){btn.disabled=false;btn.textContent='🔍 搜尋並自動標點';}
        var id=res[0], bd=res[1];
        var side=(id&&id.results&&id.results.length)?(id.results[0].side||'unknown'):'unknown';
        window.inspSide=side;
        setSideBadge(side);

        if(bd.error){if(bqS){bqS.textContent='❌ '+bd.error;bqS.className='err';bqS.style.display='block';}return;}
        var pts=bd.points||[];
        if(!pts.length){if(bqS){bqS.textContent='❌ 查無資料';bqS.className='err';bqS.style.display='block';}return;}

        var ta=document.getElementById('dataInput');
        if(!ta){if(bqS){bqS.textContent='❌ #dataInput不存在';bqS.className='err';bqS.style.display='block';}return;}
        /* ★ allPoints.y 不翻轉，保留 CF 原始座標 */
        ta.value=pts.map(function(p){return p.sheet_id+'|'+(p.url||'')+'\t'+(p.line_id||'')+'\t'+p.x+'\t'+p.y;}).join('\n');
        if(typeof window.loadData!=='function'){if(bqS){bqS.textContent='❌ loadData()不存在';bqS.className='err';bqS.style.display='block';}return;}
        window.loadData();

        setTimeout(function(){
            /* 標紅 in-place */
            var ap=window.allPoints||[];
            if(window.allMarkedIndices){window.allMarkedIndices.length=0;ap.forEach(function(_,i){window.allMarkedIndices.push(i);});}
            else{window.allMarkedIndices=ap.map(function(_,i){return i;});}
            if(window.markedPoints){window.markedPoints.clear();(window.allMarkedIndices||[]).forEach(function(i){window.markedPoints.add(i);});}
            else{window.markedPoints=new Set(window.allMarkedIndices||[]);}
            if(window.loadedRedPoints){window.loadedRedPoints.length=0;ap.forEach(function(p,i){window.loadedRedPoints.push(Object.assign({},p,{_allIdx:i}));});}
            else{window.loadedRedPoints=ap.map(function(p,i){return Object.assign({},p,{_allIdx:i});});}

            if(typeof window.updateDisplay==='function') window.updateDisplay();
            if(typeof window.refreshRedPointsDisplay==='function') window.refreshRedPointsDisplay();
            var ab=document.getElementById('actionBox'); if(ab) ab.style.display='flex';
            var sL=side==='TFT'?'🔵 TFT 側':side==='CF'?'🔴 CF 側':'⚪ 側別未知';
            var msg='✅ 共 '+pts.length+' 筆　'+sL;
            if(bd.errors&&bd.errors.length) msg+='\n⚠️ '+bd.errors.join(' / ');
            if(bqS){bqS.textContent=msg;bqS.className='ok';bqS.style.display='block';}
        }, 200);
    }).catch(function(e){
        if(btn){btn.disabled=false;btn.textContent='🔍 搜尋並自動標點';}
        if(bqS){bqS.textContent=(e.name==='TimeoutError'?'❌ 超時':'❌ '+e.message);bqS.className='err';bqS.style.display='block';}
    });
};

/* ══════════════════════════════════════════════════
   ② drawBackground：不預先翻轉底圖
      統一交給 updateDisplay wrapper 處理 CF 翻轉
   ══════════════════════════════════════════════════ */
(function(){
    var bgCache={}, bgLoad={}, bgFail={};
    function drawBG(){
        var isDark=document.body.classList.contains('dark-mode');
        ...
        ctx.fillRect(0,0,canvas.width,canvas.height);
        if(typeof PRODUCT_CONFIGS==='undefined'||typeof currentProductKey==='undefined') return;
        var prod=PRODUCT_CONFIGS[currentProductKey]; if(!prod) return;
        var RW=(typeof REAL_WIDTH!=='undefined')?REAL_WIDTH:(typeof REAL_W!=='undefined')?REAL_W:2500;
        var RH=(typeof REAL_HEIGHT!=='undefined')?REAL_HEIGHT:(typeof REAL_H!=='undefined')?REAL_H:2200;
        var SC=canvas.width/RW;

        function doLines(){
            ctx.save();
            ctx.strokeStyle='rgba(210,40,40,0.55)'; ctx.lineWidth=1; ctx.setLineDash([4,3]);
            (prod.chips||[]).forEach(function(c){
                var x1=c.x*SC, x2=(c.x+c.w)*SC;
                var y1=canvas.height-c.y*SC, y2=canvas.height-(c.y+c.h)*SC;
                if(c.x>0.5){ctx.beginPath();ctx.moveTo(x1,y2);ctx.lineTo(x1,y1);ctx.stroke();}
            ...
            ctx.setLineDash([]);
            ctx.strokeStyle=isDark?'#666':'#999'; ctx.lineWidth=1.5;
            ctx.strokeRect(0.5,0.5,canvas.width-1,canvas.height-1);
            /* TFT 方向標籤（CF 翻轉後由 wrapper 重貼） */
            (prod.chips||[]).forEach(function(c){
                var cxPx=(c.x+c.w*0.5)*SC;
                var cyPx=canvas.height-(c.y+c.h*0.5)*SC;
                var fs=Math.max(6,Math.min(c.w*SC*0.25,c.h*SC*0.25,14));
        ...
        var imgSrc=prod.image;
        if(!imgSrc){doLines();return;}
        if(bgCache[imgSrc]){
            ctx.globalAlpha=0.4; ctx.drawImage(bgCache[imgSrc],0,0,canvas.width,canvas.height); ctx.globalAlpha=1;
            doLines(); return;
        }
        if(bgFail[imgSrc]){doLines();return;}
        doLines();
        if(bgLoad[imgSrc]) return;
        bgLoad[imgSrc]=true;
        var img=new Image();
        img.onload=function(){bgCache[imgSrc]=img;bgLoad[imgSrc]=false;requestAnimationFrame(drawBG);};
        img.onerror=function(){bgFail[imgSrc]=true;bgLoad[imgSrc]=false;};
        img.src=imgSrc;
    }
    function install(){
        if(typeof PRODUCT_CONFIGS==='undefined'){setTimeout(install,200);return;}
        window.drawBackground=drawBG; drawBG();
        console.log('[drawBG v_noflip] ✓');
    }
    if(document.readyState==='complete') setTimeout(install,500);
    else window.addEventListener('load',function(){setTimeout(install,500);});
})();
...
/* ══════════════════════════════════════════════════
   ③ updateDisplay wrapper：
      CF 模式下翻轉整個 main canvas + 重貼 chip 標籤
   ══════════════════════════════════════════════════ */
(function(){
    function cfFlipCanvas(){
        if(!isCF()) return;
        var canvas=document.getElementById('canvas'); if(!canvas) return;
        var ctx=canvas.getContext('2d');
        var isDark=document.body.classList.contains('dark-mode');
        if(typeof PRODUCT_CONFIGS==='undefined'||typeof currentProductKey==='undefined') return;
        var prod=PRODUCT_CONFIGS[currentProductKey]; if(!prod||!prod.chips) return;
        var RW=(typeof REAL_WIDTH!=='undefined')?REAL_WIDTH:(typeof REAL_W!=='undefined')?REAL_W:2500;
        var SC=canvas.width/RW;
        ...
        /* 2. 重貼 chip 標籤（CF 座標：y=0 在畫布上方）
              canvas_y = (chip.y + chip.h/2) * SC
              文字正向（ctx 未 scale，直接 fillText）             */
        ctx.save();
        prod.chips.forEach(function(c){
            var cxPx=(c.x+c.w*0.5)*SC;
            var cyPx=(c.y+c.h*0.5)*SC;          /* ★ CF 公式 */
            var fs=Math.max(6,Math.min(c.w*SC*0.25,c.h*SC*0.25,14));
            ctx.font='bold '+fs+'px sans-serif';
            ctx.fillStyle=isDark?'rgba(170,205,255,0.85)':'rgba(0,60,140,0.55)';
            ctx.textAlign='center'; ctx.textBaseline='middle';
            ctx.fillText(c.label,cxPx,cyPx);
        });
        ctx.restore();
    ...
    function install(){
        if(typeof window.updateDisplay!=='function'){setTimeout(install,100);return;}
        var _orig=window.updateDisplay;
        window.updateDisplay=function(){
            _orig.apply(this,arguments);
            cfFlipCanvas();
        };
        console.log('[updateDisplay CF flip] ✓');
    }
    if(document.readyState==='complete') setTimeout(install,600);
    else window.addEventListener('load',function(){setTimeout(install,600);});
})();

/* ══════════════════════════════════════════════════
   ④ count canvas + filter canvas：CF 座標公式
      cy_CF = chip.y * scale  （y=0 在畫布上方）
      cy_TFT= H-(chip.y+chip.h)*scale  （原本公式）
   ══════════════════════════════════════════════════ */
(function(){
    /* 取得 chip 列表 */
    function getChips(){
        return (typeof getCurrentChips==='function') ? getCurrentChips() :
               (typeof PRODUCT_CONFIGS!=='undefined'&&typeof currentProductKey!=='undefined'&&PRODUCT_CONFIGS[currentProductKey]
                ? (PRODUCT_CONFIGS[currentProductKey].chips||[]) : []);
    }
    function getCanvasSize(chips){
        var RW=2500,RH=2200,n=chips.length;
    ...
    function chipY(chip,H,scale){
        return isCF() ? chip.y*scale : H-(chip.y+chip.h)*scale;
    }
    ...
    function drawCountCanvas(){
        var container=document.getElementById('p2_count_container'); if(!container) return;
        var chips=getChips(); var sz=getCanvasSize(chips);
        var W=sz.W,H=sz.H,scale=sz.scale;
        var cv=document.getElementById('p2_count_canvas');
        if(!cv){cv=document.createElement('canvas');cv.id='p2_count_canvas';
        ...
        var counts={}; chips.forEach(function(c){counts[c.label]=0;});
        var pts=window.allPoints||[];
        var idx=window.allMarkedIndices||[];
        var excl=window.excludedPoints||new Set();
        idx.forEach(function(i){
            if(excl.has&&excl.has(i)) return;
            var pt=pts[i]; if(!pt) return;
            var lbl=getRegionByChips(pt.x,pt.y);
            if(counts.hasOwnProperty(lbl)) counts[lbl]++;
        });

        chips.forEach(function(chip){
            var cx=chip.x*scale;
            var cy=chipY(chip,H,scale);          /* ★ CF 公式 */
            var cw=chip.w*scale, ch=chip.h*scale;
            var cnt=counts[chip.label]||0;
            ctx.fillStyle=cnt>0?'rgba(220,50,50,0.18)':'rgba(200,220,255,0.35)';
            ctx.fillRect(cx,cy,cw,ch);
            ctx.strokeStyle=cnt>0?'#cc3333':'#99aacc'; ctx.lineWidth=1;
            ctx.strokeRect(cx+0.5,cy+0.5,cw-1,ch-1);
            var fsL=Math.max(7,Math.min(cw*0.28,ch*0.32,13));
            ctx.fillStyle='#666'; ctx.font=fsL+'px sans-serif';
    ...
    /* ── filter canvas（含 CF click handler）── */
    function drawFilterCanvas(){
        var cv=document.getElementById('p2_filter_canvas'); if(!cv) return;
        var chips=getChips(); var sz=getCanvasSize(chips);
        var W=sz.W,H=sz.H,scale=sz.scale;
        cv.width=W; cv.height=H;
        var ctx=cv.getContext('2d'); ctx.clearRect(0,0,W,H);
        chips.forEach(function(chip){
            ...
            var cy=chipY(chip,H,scale);          /* ★ CF 公式 */
            var cw=chip.w*scale, ch=chip.h*scale;
            var on=(typeof filterState!=='undefined')?filterState[chip.label]!==false:true;
            ctx.fillStyle=on?'rgba(30,120,220,0.28)':'rgba(160,160,160,0.18)';
            ctx.fillRect(cx,cy,cw,ch);
            ctx.strokeStyle=on?'#2277cc':'#aaa'; ctx.lineWidth=on?2:1;
            ctx.strokeRect(cx+0.5,cy+0.5,cw-1,ch-1);
    ...
    /* filter click handler（CF 座標對應）*/
    function cfClickHandler(e, cv){
    ...
    /* 替換 filter canvas（清除舊 listener）並綁定新的 */
    function rebindFilter(){
        var cv=document.getElementById('p2_filter_canvas'); if(!cv) return;
        if(cv._cfv2) return;               /* 已綁定，跳過 */
        var newCv=cv.cloneNode(false);     /* 複製標籤但清除所有 listener */
        newCv._cfv2=true;
        cv.parentNode.replaceChild(newCv,cv);
        newCv.addEventListener('click',function(e){ cfClickHandler(e,newCv); });
        drawFilterCanvas();                /* 用新 canvas 重繪 */
    }

    /* updateDisplay 後更新 count/filter，並確保 filter 已綁定 CF handler */
    function installHook(){
        if(typeof window.updateDisplay!=='function'){setTimeout(installHook,100);return;}
        var _orig=window.updateDisplay;
        window.updateDisplay=function(){
            _orig.apply(this,arguments);
            /* 每次更新：確保 filter 已用 CF handler 綁定 */
            rebindFilter();
            drawCountCanvas();
            drawFilterCanvas();
        };
        /* 初次執行 */
        function waitFilter(){
            if(document.getElementById('p2_filter_canvas')){ rebindFilter(); drawCountCanvas(); }
            else{ setTimeout(waitFilter,200); }
        }
        waitFilter();
        console.log('[CF count/filter canvas v2] ✓');
    }
    if(document.readyState==='complete') setTimeout(installHook,700);
    else window.addEventListener('load',function(){setTimeout(installHook,700);});
})();

/* ── clearAll 重置 badge ── */
(function(){
    function install(){
        var _oc=window.clearAll||function(){};
        window.clearAll=function(){
            _oc();
            window.inspSide=null;
            var b=document.getElementById('sideBadge'); if(b) b.style.display='none';
        };
    }
    if(document.readyState==='complete') setTimeout(install,800);
    else window.addEventListener('load',function(){setTimeout(install,800);});
})();

console.log('[CF v2 完整修正] 安裝排程完成 ✓');
})();
