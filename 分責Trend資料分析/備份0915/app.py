...
def query_rawdata_db(products, periods, period_type, defect_group):
    ...
    # Build SELECT: A.col for RESPONSE cols, B.PA, B.PA_DAY for OPER cols
    ...
    def do_POST(self):
        ...
        if path == "/api/fma":
            ...
            if not products or not periods:
                self.send_json(200, {"data":"", "error":"products/periods is empty"}); return
            try:
                data = query_rawdata_db(products, periods, period_type, defect_group)
                self.send_json(200, {"data": data, "error": None})
            except Exception as e:
                self.send_json(200, {"data":"", "error": str(e)})
            ...
            urls = body.get("urls", [])
            if not urls:
                self.send_json(200, {"data":"", "error":"urls is empty"}); return
            try:
                all_rows, headers = [], None
                for url in urls:
                    rows = fetch_rawdata_http(url)
                    if not rows: continue
                    if headers is None: headers=rows[0]; all_rows.extend(rows[1:])
                    else: all_rows.extend(rows[1:])
                lines = []
                if headers: lines.append('\t'.join(headers))
                for row in all_rows: lines.append('\t'.join(row))
...
if __name__ == "__main__":
    ...
    print(f"✅ FMA Proxy 啟動，監聽 http://localhost:{PORT}")
    print(f"   FMA:     MySQL CELODS.H_FMA_COMPOSITION")
    print(f"   RAWDATA: MySQL RESPONSE.H_FIRST_RHNVS + OPER.H_SHEET_OPER_FEOL")
