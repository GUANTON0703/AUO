...
【修改說明 v3】
  - update_record：移除不存在的 txt_db.find_by_created_at 呼叫
    改由 txt_db.update() 回傳值判斷是否成功
  - 修正 photo_urls 縮排錯誤（IndentationError）

【修改說明 v4】★
  - serve_photo：修正雙重 response header 問題
    原本：wfile.write 失敗後還呼叫 send_error(500)，導致回應頭送兩次
    修正：參照 summary_bg_image 的正確模式，將讀檔與送出分開 try，
          BrokenPipeError / ConnectionResetError 視為瀏覽器取消而非錯誤
    順便：log 增加 path 資訊，方便追查是哪個檔案出問題
"""
...
# ★ v4 修正：serve_photo 參照 summary_bg_image 的正確模式
def serve_photo(handler, params):
    path = unquote(params.get('path', ''))
    # ── 路徑檢查（在 try 外，確保 not-exist 回 404 而非 500）─────
    if not path or not os.path.exists(path):
        log(f'⚠️  照片不存在：{path}')
        handler.send_json({'error': '照片不存在'}, 404)
        return
    ...
    try:
        ...
        try:
            ...
            # 連線在傳輸中被關閉（瀏覽器 cancel / 超時），非真正錯誤，不 send_error
            log(f'⚠️  圖片傳輸中斷（連線已關閉）：{os.path.basename(path)}')
    except Exception as e:
        # 這裡只會抓到「讀檔失敗」，headers 尚未送出，可安全呼叫 send_error
        log(f'❌ 照片讀取失敗：{os.path.basename(path)} → {type(e).__name__}: {e}')
        try:
...
def update_record(handler, data):
    try:
        ...
        # ★ v3 修正：移除不存在的 find_by_created_at，直接由 update() 回傳值判斷
            ...
            for dxml in draw_files:
                ...
                for anchor_elem in draw_root:
                    ...
                    if row_tag is None:
                        continue
                    try:
                        row_0idx = int(row_tag.text)
                    except Exception:
                        continue
                    pic_elem = anchor_elem.find(f".//{{{XDR}}}pic")
                    if pic_elem is None:
                        continue
                    blipFill = pic_elem.find(f".//{{{XDR}}}blipFill")
                    if blipFill is None:
                        import xml.etree.ElementTree as ET2
                        blipFill = anchor_elem.find(
                            f".//{{{XDR}}}blipFill") or anchor_elem.find(
                            ".//{http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing}blipFill")
                    blip = None
                    if blipFill is not None:
                        for child in blipFill:
                            if child.tag.endswith('}blip') or child.tag == 'blip':
                                blip = child; break
                    if blip is None:
                        blip = anchor_elem.find(
                            ".//{http://schemas.openxmlformats.org/drawingml/2006/main}blip")
                    if blip is None:
                        continue
                    embed = blip.get(f"{{{R}}}embed") or blip.get("r:embed") or blip.get("embed")
                    ...
                    row_key   = str(row_0idx)
                    result.setdefault(row_key, []).append(img_bytes)
    except Exception as ex:
        log(f"[圖片] zip 解析失敗：{ex}")
    return result


def bulk_update(handler, data):
    try:
        records = data.get('records', data.get('updatedRecs', []))
        ...
        handler.send_json({'success': True, 'updated': updated, 'errors': errors})
    except Exception as e:
        log(f'❌ 批次更新失敗：{e}')
        handler.send_json({'success': False, 'error': str(e)}, 500)
...
def machine_all_positions(handler, params):
    try:
        if not machine_db.is_loaded():
            handler.send_json({'success': False, 'error': '機台資料未載入'}, 503); return
        handler.send_json({'success': True, 'records': machine_db.records,
                           'headers': machine_db.headers})
    except Exception as e:
        log(f'❌ 機台資料讀取失敗：{e}')
        handler.send_json({'success': False, 'error': str(e)}, 500)


def machine_map_image(handler, params):
    try:
        if not machine_db.is_loaded():
            handler.send_json({'success': False, 'error': '機台資料未載入'}, 503); return
        import base64
        img_path = getattr(machine_db, 'image_path', None)
        if not img_path or not os.path.exists(img_path):
            handler.send_json({'success': False, 'error': '機台圖片不存在'}); return
        with open(img_path, 'rb') as f:
            img_b64 = base64.b64encode(f.read()).decode()
        ext = img_path.rsplit('.', 1)[-1].lower()
        mime = {'jpg': 'image/jpeg', 'jpeg': 'image/jpeg', 'png': 'image/png'}.get(ext, 'image/jpeg')
        handler.send_json({'success': True, 'image': f'data:{mime};base64,{img_b64}'})
    except Exception as e:
        log(f'❌ 機台圖片讀取失敗：{e}')
        handler.send_json({'success': False, 'error': str(e)}, 500)


def summary_bg_image(handler):
    """Summary Y分布圖 底圖（27PIC.jpg）"""
    img_path = r'\\tw100049089\00_MyAgent\Roller Mark 背污逐站檢\前後段拆分版\HTA版本\27PIC.jpg'
    try:
        if not os.path.exists(img_path):
            handler.send_json({'success': False, 'error': '底圖不存在: ' + img_path}, 404)
            return
        with open(img_path, 'rb') as f:
            data = f.read()
        handler.send_response(200)
